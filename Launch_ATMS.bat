...
:: Auto-register for Windows Startup
set "StartupFolder=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup"
if not exist "%StartupFolder%\Launch_ATMS.bat" (
    copy "%~f0" "%StartupFolder%\Launch_ATMS.bat" /Y >nul 2>&1
    echo Registered for auto-start on Windows login.
)
